...
def show_res(rsp_json):
    ...
    try:
        ...
        # FIXED SYNTX ERROR HERE: Enclosed the assignment expression in parentheses
        if not error_msg and (Ghent := rsp_json.get('garena_response', {})):
            if isinstance(Ghent, dict) and Ghent.get('error'):
                error_msg = Ghent.get('error')
...
def Eat_To_Access_Login():
    user_input = input('\033[92m- EnTer EAT Token or URL : \033[0m').strip()
    ...
    if "eat=" in user_input:
        try:
            ...
            pass
    try:
        animate_print("[*] Processing Token Login...", speed=0.03)
        res = requests.get("https://project-m0dtj.vercel.app/token", params={'eat_token': clean_token}, timeout=20)
        print(f"\n\033[92m[🎉] Result:\n{res.text}\033[0m")
...
def MenU():
    ...
    while True:
        ...
        print(f" {G}[8]{X} - EAT TO ACCESS (Token Login)") 
        print(f" {R}[0]{X} - ExiT")
        print(f"{G}=========================================================================={X}")
        ...
        input(f'\n{Y}- PrEss EnEnter To REturn To Main MEnu...{X}')
